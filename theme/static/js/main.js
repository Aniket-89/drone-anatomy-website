...
const root = document.documentElement; /* Access root element for scroll position */

// Update scroll position variable on scroll
window.addEventListener('scroll', function() {
  root.style.setProperty('--scroll-y', window.scrollY + 'px'); // Update scroll variable
  
  // Check for unsticky behavior (optional)
  if (textList.scrollTop > 0) {
    stickySection.classList.add('unsticky');
  } else {
    stickySection.classList.remove('unsticky');
  }
});